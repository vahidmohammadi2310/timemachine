...
static void gtk_meterscale_size_request        (GtkWidget *widget, GtkRequisition *requisition);
static void gtk_meterscale_size_allocate       (GtkWidget *widget, GtkAllocation *allocation);
static gint gtk_meterscale_expose              (GtkWidget *widget, GdkEventExpose *event);
static float iec_scale(float db);
static void meterscale_draw_notch_label(GtkMeterScale *meterscale, float db, int mark, PangoRectangle *last_label_rect);
...
static void gtk_meterscale_realize (GtkWidget *widget) {
  ...
  if (meterscale->direction & GTK_METERSCALE_LEFT || meterscale->direction & GTK_METERSCALE_RIGHT) {
    meterscale->min_width = rect.width + METERSCALE_MAX_FONT_SIZE;
  }

  if (meterscale->direction & GTK_METERSCALE_TOP || meterscale->direction & GTK_METERSCALE_BOTTOM) {
...
static void meterscale_draw_notch_label(GtkMeterScale *meterscale, float db, int mark, PangoRectangle *last_label_rect) {
  ...
  if (meterscale->direction & GTK_METERSCALE_LEFT || meterscale->direction & GTK_METERSCALE_RIGHT) {
    ...
    pos = length - length * (iec_scale(db) - meterscale->iec_lower) / (meterscale->iec_upper - meterscale->iec_lower);
    vertical = 1;
  } else {
    length = widget->allocation.width - 2;
    width = widget->allocation.height - 2;
    pos = length * (iec_scale(db) - meterscale->iec_lower) / (meterscale->iec_upper - meterscale->iec_lower);
  }
...
static void meterscale_draw_notch(GtkMeterScale *meterscale, float db, int mark) {
  ...
  if (meterscale->direction & GTK_METERSCALE_LEFT) {
    gdk_draw_rectangle(widget->window, widget->style->black_gc, TRUE, 0, pos, mark, 1);
  }

  if (meterscale->direction & GTK_METERSCALE_RIGHT) {
    gdk_draw_rectangle(widget->window, widget->style->black_gc, TRUE, width - mark + 1, pos, mark, 1);
  }

  if (meterscale->direction & GTK_METERSCALE_TOP) {
    gdk_draw_rectangle(widget->window, widget->style->black_gc, TRUE, pos+1, 1, 1, mark);
  }

  if (meterscale->direction & GTK_METERSCALE_BOTTOM) {
    gdk_draw_rectangle(widget->window, widget->style->black_gc, TRUE, pos+1, width -mark + 1, 1, mark);
  }
}
