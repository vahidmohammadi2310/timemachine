...
GtkWidget *make_meter(float lower, float upper) {
    GtkAdjustment *adjustment = (GtkAdjustment *) gtk_adjustment_new(-60.0f, lower, upper, 0.0, 0.0, 0.0);
...
GtkWidget *create_window(const char *title) {
    ...
    g_signal_connect((gpointer) window, "delete_event", G_CALLBACK(on_window_delete_event), NULL);
    g_signal_connect((gpointer) togglebutton1, "clicked", G_CALLBACK(on_togglebutton1_clicked), NULL);
